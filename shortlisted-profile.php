...
<?php
$viewCount=db_scalar("SELECT COUNT(*) FROM tbl_shortlist WHERE sl_mem='$userDATA[reg_id]'");
$viewCount2=db_scalar("SELECT COUNT(*) FROM tbl_shortlist WHERE sl_by='$userDATA[reg_id]'");
?>  
  
  <nav class="navd_1">   
  <label class="msg-box" for="profile">Shortlisted My Profile (<?=$viewCount?>)</label>
<label class="msg-box" for="settings">Shortlisted Profile By Me (<?=$viewCount2?>)</label>
  </nav>
...
<?php
$sql="SELECT * FROM tbl_shortlist WHERE sl_mem='$userDATA[reg_id]'";
...
$sql="SELECT * FROM tbl_registration WHERE reg_id='$recViewed[sl_by]' ";
...
<div class="clearfix"></div>
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 btn-gryes mp-top2"><a target="_blank" href="other-profile.php?mem=<?=$recView['reg_id']?>" class="f-btn"> Full Profile</a></div>
