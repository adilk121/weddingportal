...
<?php
$viewCount=db_scalar("SELECT COUNT(*) FROM tbl_profile_view WHERE pro_view_to_mem_id='$userDATA[reg_id]'");
$viewCount2=db_scalar("SELECT COUNT(*) FROM tbl_profile_view WHERE pro_view_by_mem_id='$userDATA[reg_id]'");
?>  
  
  <nav class="navd_1">   
  <label class="msg-box" for="profile">Viewed My Profile (<?=$viewCount?>)</label>
  <label class="msg-box" for="settings">Profile Viewed By Me (<?=$viewCount2?>)</label>
  </nav>
...
<?php
$sql="SELECT * FROM tbl_profile_view WHERE pro_view_to_mem_id='$userDATA[reg_id]'";
...
$sql="SELECT * FROM tbl_registration WHERE reg_id='$recViewed[pro_view_by_mem_id]' ";
