...
if($st==1){
$sql="UPDATE  tbl_search_city SET m_status='Inactive' WHERE m_id='$catID' ";	
$res=db_query($sql);	
if($res>0){
$_SESSION["msg"]="Selected state is deactivated successfully.";	
}	
}else{
$sql="UPDATE  tbl_search_city SET m_status='Active' WHERE m_id='$catID' ";	
$res=db_query($sql);	
if($res>0){
...
header("location:manage-search-city.php");
exit;
}
...
if(is_post_back()) {
	...
	if(is_array($arr_ids)) {
		...
		if(isset($_REQUEST['Activate']) || isset($_REQUEST['Activate_x']) ) {
			db_query("update  tbl_search_city set m_status='Active' where m_id in ($str_ids)");
			$_SESSION["msg"]="selected states are activated. ";
		} else if(isset($_REQUEST['Deactivate']) || isset($_REQUEST['Deactivate_x']) ) {
			db_query("update  tbl_search_city set m_status='Inactive' where m_id in ($str_ids)");
						$_SESSION["msg"]="selected states are deactivated. ";
		}else if(isset($_REQUEST['set_home']) || isset($_REQUEST['set_home_x']) ) {
			db_query("update  tbl_search_city set category_for_home='Yes' where m_id in ($str_ids)");
						$_SESSION["msg"]="selected categories is set for home. ";
		}else if(isset($_REQUEST['remove_home']) || isset($_REQUEST['remove_home_x']) ) {
			db_query("update  tbl_search_city set category_for_home='No' where m_id in ($str_ids)");
						$_SESSION["msg"]="selected categories is removed from home. ";
		}else if(isset($_REQUEST['Delete']) || isset($_REQUEST['Delete_x']) ) {
			db_query("Delete FROM  tbl_search_city where m_id in ($str_ids)");
						$_SESSION["msg"]="selected state is deleted. ";
		}
	...
	if(isset($_REQUEST['Update'])){
		...
		db_query("update  tbl_search_city set c_order_by ='$value' where m_id='$key'");
		}
	}
...
$sql = "select * from   tbl_search_city   where 1 ";
...
<a href="addedit-search-city.php?id=0"><button id="btn-go-back" type="button" class="btn btn-labeled btn-inverse m-b-5 pull-right">
<span class="btn-label"><i class="fa fa-plus" aria-hidden="true"></i>
</span>Add City
</button></a>

<span class="count-num">Total : <?=db_scalar("SELECT COUNT(m_id) FROM  tbl_search_city")?></span>
                     ...
                     <div class="panel panel-bd lobidrag" data-edit-title='false' data-close='false' data-reload='false'>
                        ...
                        <div class="panel-heading">
                           <div class="btn-group" id="buttonexport">
                              <a href="#">
                                 <h4 class="font-black">City List</h4>
                              </a>
                           </div>
                        </div>
...
<a href="manage-search-city.php?st=1&pid=<?=$line_raw["m_id"]?>"><span class="label-custom label label-default">Active</span></a>
<?php }else{?>
<a href="manage-search-city.php?st=0&pid=<?=$line_raw["m_id"]?>"><span class="label-danger label label-default">Inactive</span></a>
<?php }?>
...
<td class="text-center v-middle">
<a href="addedit-search-city.php?id=<?=$line_raw["m_id"]?>"><button type="button" class="btn btn-add btn-sm" ><i class="fa fa-pencil"></i></button>
</a>                                          
</td>
