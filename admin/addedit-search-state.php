...
$sql = "update  tbl_search_state set        
				m_title='$m_title',
				m_status='$m_status'
				where m_id = '$m_id' ";
...
$_SESSION["msg"]="State Is Updated Successfully !";
header("Location:addedit-search-state.php?id=$_REQUEST[id]");
exit;
//*************** UPDATE EXISTING CATEGORY END ************************//
}else{

////////////****************** IMAGE RESIZING END HERE *****************************//
$sql = "insert into  tbl_search_state set 
				m_title='$m_title',
				m_status='$m_status'
				";
				
db_query($sql);
$_SESSION["msg"]="State Is added successfully !";
header("Location:addedit-search-state.php?id=$_REQUEST[id]");
exit;
...
if($m_id!='') {
	$result = db_query("select * from  tbl_search_state where m_id = '$m_id'");
...
<a href="manage-state.php"><button id="btn-go-back" type="button" class="btn btn-labeled btn-inverse m-b-5 pull-right">
