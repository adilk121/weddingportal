...
<input type="radio" id="settings" value="2" name="tractor" /> 
  ...
  <nav class="navd_1">   
  <label class="msg-box" for="profile">Received</label>
  <label class="msg-box" for="settings">Sent</label>
  </nav>
  
  <article class='uno'>
  
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padd" id="received-load-area">
<?php
$sql="SELECT * FROM tbl_registration AS r INNER JOIN tbl_msg_box AS mb ON r.reg_id=mb.msg_from_mem_id  WHERE r.reg_status='Active' AND mb.msg_to_mem_id='$userDATA[reg_id]' AND (mb.msg_status='Pending' OR mb.msg_status='PendingAgain') ORDER BY reminder_counter desc";

$dataMemberRecieved=db_query($sql);
$countMemberRecieved=mysqli_num_rows($dataMemberRecieved);
if($countMemberRecieved){
while($recMemberRecieved=mysqli_fetch_array($dataMemberRecieved)){	
?>   
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mp_1d"> 
   ...
   <h2 class="nm-acep"><a target="_blank" href="other-profile.php?mem=<?=$recMemberRecieved['reg_id']?>&receiver=<?=$userDATA[reg_id]?>"><?=$recMemberRecieved['reg_name']?> <span class="spn-sty1">(<?="R".$recMemberRecieved['reg_id']?>)</span></a></h2>
   </div>
...
<div class="clearfix"></div>
              ...
              <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 btn-gryes mp-top2"><a target="_blank" href="other-profile.php?mem=<?=$recMemberRecieved['reg_id']?>&receiver=<?=$userDATA[reg_id]?>" class="f-btn"> Full Profile</a></div>
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 btn-gryes"><a href="Javascript:void(0)" onClick="change_status('<?=$recMemberRecieved['msg_id']?>','Accepted')" class="a-btn">Accept</a></div>
             <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 btn-gryes"><a href="Javascript:void(0)" onClick="change_status('<?=$recMemberRecieved['msg_id']?>','Declined')" class="d-btn">Decline</a></div>
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-padd" id="pending-load-area">
<?php
